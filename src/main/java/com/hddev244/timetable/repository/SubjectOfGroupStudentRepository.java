...
public interface SubjectOfGroupStudentRepository extends JpaRepository<SubjectOfGroupStudentEntity , Long> {
    ...
    @Query("SELECT s FROM SubjectOfGroupStudentEntity s WHERE s.block.id = ?1 AND s.semester.id = ?2 AND s.subject.id = ?3 AND s.groupStudent.major.id = ?4 AND s.groupStudent.schoolYear.id = ?5")
    List<SubjectOfGroupStudentEntity> findByBlockIdAndSemesterIdAndSubjectId(Long blockId, Long semesterId, String subject_id, String major_id, Integer schoolYear_id);
}
